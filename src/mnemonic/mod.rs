//! This file imported, updated to Rust 2018, and adapted to use alternate crypto libraries from
//! https://github.com/rust-bitcoin/rust-bip39 it is subject to the CC0 license and created by
//! Steven Roose <steven@stevenroose.org>
//! You should have received a copy of the CC0 Public Domain Dedication
//! along with this software.
//! If not, see <http://creativecommons.org/publicdomain/zero/1.0/>.
//! This is only compiled if the bip39 feature is enabled.
...
impl Mnemonic {
    ...
    pub fn language_of(s: &str) -> Result<Language, Bip39Error> {
        ...
        if first_word.is_empty() {
            return Err(Bip39Error::BadWordCount(0));
        }
        ...
        for word in s.split_whitespace() {
            ...
            if langs.is_empty() {
                return Err(Bip39Error::UnknownWord(word.to_owned()));
            }
        }
        Err(Bip39Error::AmbiguousWordList(langs))
    }
...
impl FromStr for Mnemonic {
    ...
    fn from_str(s: &str) -> Result<Mnemonic, Bip39Error> {
        Mnemonic::parse(s)
    }
}
...
mod tests {
    ...
    fn test_invalid_engish() {
        ...
        assert_eq!(
            ...
            Err(Bip39Error::UnknownWord("getter".to_owned()))
        );
    ...
    fn test_invalid_entropy() {
        ...
        assert_eq!(
            ...
            Err(Bip39Error::BadEntropyBitCount(136))
        );

        //less than 128 bits
        assert_eq!(
            Mnemonic::from_entropy(&[b'x'; 4]),
            Err(Bip39Error::BadEntropyBitCount(32))
        );

        //greater than 256 bits
        assert_eq!(
            Mnemonic::from_entropy(&[b'x'; 36]),
            Err(Bip39Error::BadEntropyBitCount(288))
        );
    }
